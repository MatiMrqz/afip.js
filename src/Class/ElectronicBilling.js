    ...
    async createVoucher(data, returnResponse = false) {
        ...
        if (returnResponse === true) {
            ...
            if (Array.isArray(results.FeDetResp.FECAEDetResponse)) {
                return results.FeDetResp.FECAEDetResponse.map(r => {
                    return {
                        'N': r.CbteDesde,
                        'CAE': r.CAE,
                        'CAEFchVto': this.formatDate(r.CAEFchVto)
                    }
                })
            }
            return {
    ...
    async _checkErrors(operation, results) {
        ...
        if (operation === 'FECAESolicitar' && res.FeDetResp) {
            if (Array.isArray(res.FeDetResp.FECAEDetResponse)) {
                ...
                res.FeDetResp.FECAEDetResponse.forEach(r => {
                    if (r.Observaciones && r.Resultado !== 'A') {
                        if (!res.Errors) {
                            res.Errors = [...r.Observaciones.Obs]
                        } else {
                            res.Errors = [...res.Errors, ...r.Observaciones.Obs]
                        }
                    }
                })
            } else {
                if (res.FeDetResp.FECAEDetResponse.Observaciones && res.FeDetResp.FECAEDetResponse.Resultado !== 'A') {
                    res.Errors = [...res.FeDetResp.FECAEDetResponse.Observaciones.Obs];
                }
            }
        ...
        if (res.Errors) {
            const err = Array.isArray(res.Errors) ? res.Errors[0] : res.Errors;
